...
from estimator.prob import guessing_set_and_hit_probability


# EDIT: new function to account for rotations in power of two cyclotomics
# Borrowed from https://github.com/TabOg/mlwe-hybrids
...
class PrimalMeetProjected:
    ...
    def cost_meet_lwe_unbalanced(search_space: SparseTernary, r: int, sigma: float, prob_adm: float):
        ...
        def cost_epsilon(epsilon):
            ...
            prob_adm = RR(mitm_babai_probability_proj(r[:-dim_babai], sigma, fast=dim_babai))
...
#            log_bet += log(prob_adm)
    ...
    def cost(
        ...
        while hw <= min(h, zeta):
            ...
            # 3. Determine cost of doing Meet LWE.
            if params.Xs.ones is None:
                cost_meet = PrimalMeetProjected.cost_meet_lwe_unbalanced(search_space, r, sigma, prob_adm=None)
            else:
                cost_meet = PrimalMeetProjected.cost_meet_lwe(search_space, r, sigma, prob_adm=None)

            idx = ( len(r) - cost_meet["eta"] )
            prob_np = RR(babai_gaussian(r[idx:], sigma))
            probability = (
                prob_hw  # prob. secret splits with given weights. Note: p_HW ~ 1/n roughly
                * prob_np  # prob. correct guess lifts with np. Note: p_NP ~ 1.0 (p_NP > p_adm).
                # * prob_adm  # prob. s = s1 + s2 has both (s1, s2) in the same bucket. (now part of MEET-LWE)
            ...
            cost = Cost({
                ...
                "h_": hw, "h_1": cost_meet["h_1"], "h_2": cost_meet["h_2"],
                "epsilon": cost_meet["epsilon"], "d_": cost_meet["dim_babai"],
                "|S|": search_space, "prob": probability, "eta": cost_meet["eta"]
            })
    ...
    def __call__(
        ...
        **kwds,
        ...
        cost["tag"] = "primal-meet-hybrid-proj"
        cost["problem"] = params
        return cost.sanity_check()

    __name__ = "primal_meet_projected"


primal_meet = PrimalMeet()
primal_meet_projected = PrimalMeetProjected()
